...
class CAMixer(nn.Module):
    def __init__(self, dim, window_size=8, bias=True, is_deformable=True, ratio=0.5, training=True):
        super().__init__()    
    ...
    def forward(self,x,condition_global=None, mask=None):
        N,C,H,W = x.shape
...
class CATransformerBlock(nn.Module):
    def __init__(self, dim, window_size, ratio, training, num_channel_heads,  ffn_expansion_factor, bias, LayerNorm_type):
        ...
        self.spatial_attn = CAMixer(dim,window_size=window_size,ratio=ratio, training=training)
    ...
    def forward(self, x, global_condition=None):
        if self.training:
            ...
            y, decision = self.spatial_attn(self.norm3(x), global_condition)
            x  = x + y
            ...
            x = x + self.channel_attn(self.norm1(x))
            x = x + self.channel_ffn(self.norm2(x))
            x = x + self.spatial_attn(self.norm3(x), global_condition)
            x = x + self.spatial_ffn(self.norm4(x))
            return x
...
class XRestormerLayer(nn.Module):
    def __init__(self, dim, depth, window_size, ratio, training, num_channel_heads, ffn_expansion_factor, bias, LayerNorm_type):
        super(XRestormerLayer, self).__init__()
        self.layer = nn.Sequential(*[CATransformerBlock(dim=dim, window_size = window_size, ratio = ratio, training = training, num_channel_heads=num_channel_heads,  ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type) for i in range(depth)])
        self.training = training
...
class CAPromptXRestormerEff(nn.Module):
    def __init__(self,
        inp_channels=3,
        out_channels=3,
        dim = 48,
        ...
        super(CAPromptXRestormerEff, self).__init__()
        print("Initializing XRestormer")
        self.scale = scale

        self.patch_embed = OverlapPatchEmbed(inp_channels, dim)
        self.encoder_level1 = XRestormerLayer(dim=dim, window_size = window_size, ratio = ratio, training = training, num_channel_heads=channel_heads[0],  ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type, depth=num_blocks[0])

        self.down1_2 = Downsample(dim) ## From Level 1 to Level 2
        self.encoder_level2 = XRestormerLayer(dim=int(dim*2**1), window_size = window_size, ratio = ratio, training = training,  num_channel_heads=channel_heads[1],  ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type, depth=num_blocks[1])

        self.down2_3 = Downsample(int(dim*2**1)) ## From Level 2 to Level 3
        self.encoder_level3 = XRestormerLayer(dim=int(dim*2**2), window_size = window_size, ratio = ratio, training = training,  num_channel_heads=channel_heads[2],  ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type, depth=num_blocks[2])

        self.down3_4 = Downsample(int(dim*2**2)) ## From Level 3 to Level 4
        self.latent = XRestormerLayer(dim=int(dim*2**3), window_size = window_size, ratio = ratio, training = training,  num_channel_heads=channel_heads[3], ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type, depth=num_blocks[3])
        ...
        self.decoder_level3 = XRestormerLayer(dim=int(dim*2**2), window_size = window_size, ratio = ratio, training = training,  num_channel_heads=channel_heads[2], ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type, depth = num_blocks[2])


        self.up3_2 = Upsample(int(dim*2**2)) ## From Level 3 to Level 2
        self.reduce_chan_level2 = nn.Conv2d(int(dim*2**2), int(dim*2**1), kernel_size=1, bias=bias)
        self.decoder_level2 = XRestormerLayer(dim=int(dim*2**1), window_size = window_size, ratio = ratio, training = training,  num_channel_heads=channel_heads[1], ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type, depth = num_blocks[1])

        self.up2_1 = Upsample(int(dim*2**1))  ## From Level 2 to Level 1  (NO 1x1 conv to reduce channels)

        self.decoder_level1 = XRestormerLayer(dim=int(dim*2**1), window_size = window_size, ratio = ratio, training = training,  num_channel_heads=channel_heads[0], ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type, depth = num_blocks[0])

        self.refinement = XRestormerLayer(dim=int(dim*2**1), window_size = window_size, ratio = ratio, training = training,  num_channel_heads=channel_heads[0],  ffn_expansion_factor=ffn_expansion_factor, bias=bias, LayerNorm_type=LayerNorm_type, depth=num_refinement_blocks)
    ...
    def forward(self, inp_img):
        ...
        if self.training:
            decision_avg = 0
            out_enc_level1, decision = self.encoder_level1(inp_enc_level1, condition_global)
            inp_enc_level2 = self.down1_2(out_enc_level1)
            decision_avg += decision
            

            # Encoder2
            out_enc_level2, decision = self.encoder_level2(inp_enc_level2, condition_global_level2)
            inp_enc_level3 = self.down2_3(out_enc_level2)
            decision_avg += decision
            
            # Encoder3
            out_enc_level3, decision = self.encoder_level3(inp_enc_level3, condition_global_level3)
            inp_enc_level4 = self.down3_4(out_enc_level3)
            decision_avg += decision
            
            # Bottleneck
            latent, decision = self.latent(inp_enc_level4, condition_global_level4)
            decision_avg += decision
            ...
            out_dec_level3, decision = self.decoder_level3(inp_dec_level3, condition_global_level3)
            decision_avg += decision
            ...
            out_dec_level2, decision = self.decoder_level2(inp_dec_level2, condition_global_level2)
            decision_avg += decision
            ...
            out_dec_level1, decision = self.decoder_level1(inp_dec_level1, condition_global)
            decision_avg += decision

            out_dec_level1, decision = self.refinement(out_dec_level1, condition_global)
            decision_avg += decision
            out_dec_level1 = self.output(out_dec_level1) + inp_img

            decision_avg /= 8

            return out_dec_level1, decision_avg
        else:
            out_enc_level1 = self.encoder_level1(inp_enc_level1, condition_global)
            inp_enc_level2 = self.down1_2(out_enc_level1)
            out_enc_level2 = self.encoder_level2(inp_enc_level2, condition_global_level2)
            inp_enc_level3 = self.down2_3(out_enc_level2)
            out_enc_level3 = self.encoder_level3(inp_enc_level3, condition_global_level3)
            inp_enc_level4 = self.down3_4(out_enc_level3)
            latent = self.latent(inp_enc_level4, condition_global_level4)
            ...
            inp_dec_level3 = torch.cat([inp_dec_level3, out_enc_level3], 1)
            inp_dec_level3 = self.reduce_chan_level3(inp_dec_level3)
            out_dec_level3 = self.decoder_level3(inp_dec_level3, condition_global_level3)
            ...
            inp_dec_level2 = torch.cat([inp_dec_level2, out_enc_level2], 1)
            inp_dec_level2 = self.reduce_chan_level2(inp_dec_level2)
            out_dec_level2 = self.decoder_level2(inp_dec_level2, condition_global_level2)
            ...
            inp_dec_level1 = self.up2_1(out_dec_level2)
            inp_dec_level1 = torch.cat([inp_dec_level1, out_enc_level1], 1)
            out_dec_level1 = self.decoder_level1(inp_dec_level1, condition_global)

            out_dec_level1 = self.refinement(out_dec_level1, condition_global)
            out_dec_level1 = self.output(out_dec_level1) + inp_img
...
if __name__ == "__main__":
    ...
    if training:
        ...
        print("Complex ratio: %.2f"%decision.item())
    else:
        y = model(x)
        print("output shape", y.shape)
