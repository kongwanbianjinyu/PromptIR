...
class PromptBlock(nn.Module):
    def __init__(self,  window_size, overlap_ratio, num_channel_heads, num_spatial_heads, 
        ...
        # prompt interaction
        self.attn = TransformerBlock(dim=lin_dim + prompt_dim, window_size = window_size, 
...
class PromptXRestormer(nn.Module):
    def __init__(self,
        ...
        self.prompt = prompt
        if prompt:
            self.prompt1 = PromptBlock(prompt_dim=64,prompt_len=5,prompt_size = 64,lin_dim = 96, 
                                       window_size = 8, overlap_ratio = 0.5, num_channel_heads = 1, 
                                       num_spatial_heads = 2, spatial_dim_head = spatial_dim_head, ffn_expansion_factor = ffn_expansion_factor, 
                                       bias = bias, LayerNorm_type = LayerNorm_type)
            self.prompt2 = PromptBlock(prompt_dim=128,prompt_len=5,prompt_size = 32,lin_dim = 192, 
                                       window_size = 8, overlap_ratio = 0.5, num_channel_heads = 1, 
                                       num_spatial_heads = 4, spatial_dim_head = spatial_dim_head, ffn_expansion_factor = ffn_expansion_factor, 
                                       bias = bias, LayerNorm_type = LayerNorm_type)
            self.prompt3 = PromptBlock(prompt_dim=320,prompt_len=5,prompt_size = 16,lin_dim = 384, 
                                       window_size = 8, overlap_ratio = 0.5, num_channel_heads = 1, 
                                       num_spatial_heads = 8, spatial_dim_head = spatial_dim_head, ffn_expansion_factor = ffn_expansion_factor, 
                                       bias = bias, LayerNorm_type = LayerNorm_type)
    ...
    def forward(self, inp_img):
        ...
        if self.prompt:
            out_dec_level3 = self.prompt2(out_dec_level3)
        ...
        if self.prompt:
            out_dec_level2 = self.prompt1(out_dec_level2)
