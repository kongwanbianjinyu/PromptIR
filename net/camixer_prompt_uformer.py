...
class CATransformerBlock(nn.Module):
    def __init__(self, dim, win_size=8, mlp_ratio=4.,act_layer=nn.GELU, drop=0.,drop_path=0.,
                 norm_layer=nn.LayerNorm, ratio=0.5, training = True):
        super(CATransformerBlock,self).__init__()
        
        self.dim = dim
        self.norm1 = norm_layer(dim)
        self.mixer = CAMixer(dim,window_size=win_size,ratio=ratio, training=training)
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp =  LeFF(dim,mlp_hidden_dim,act_layer=act_layer, drop=drop)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
    ...
    def forward(self,x,condition_global=None):
        if self.training:
            shortcut = x
            x = self.norm1(x)
            x, decision = self.mixer(x,condition_global)
            x = shortcut + self.drop_path(x)
            x = x + self.drop_path(self.mlp(self.norm2(x)))
            return x, decision
        else:
            shortcut = x
            x = self.norm1(x)
            x = self.mixer(x,condition_global)
            x = shortcut + self.drop_path(x)
            x = x + self.drop_path(self.mlp(self.norm2(x)))
            return x 
...
class BasicUformerLayer(nn.Module):
    def __init__(self, dim, output_dim,  depth, num_heads, win_size,
        ...
        self.blocks = nn.ModuleList([
            CATransformerBlock(dim = dim, win_size=win_size, mlp_ratio=mlp_ratio, 
                               act_layer=nn.GELU, drop=drop, drop_path=drop_path,)
    ...
    def forward(self, x, condition_global=None):
        if self.training:
            ...
            for blk in self.blocks:
                x, decision = blk(x, condition_global = condition_global)
                avg_decision += decision
            avg_decision /= len(self.blocks)
            return x, avg_decision
        else:
            for blk in self.blocks:
                x = blk(x, condition_global = condition_global)
            return x
...
class PromptBlock(nn.Module):
    def __init__(self,prompt_dim=128,prompt_len=5,prompt_size = 96,lin_dim = 192, 
                 num_heads=4,win_size=8,mlp_ratio=4.,qkv_bias=True,qk_scale=None,drop=0.,attn_drop=0.,drop_path=0.,
                 norm_layer=nn.LayerNorm,token_projection='linear',token_mlp='ffn',modulator=False,cross_modulator=False):
        ...
        # prompt interaction
        self.attn = LeWinTransformerBlock(dim=lin_dim + prompt_dim,
...
class CAPromptUformerIR(nn.Module):
    def __init__(self, img_size=256, in_chans=3, dd_in=3,
                 ...
                 cross_modulator=False, prompt = True, training = True, **kwargs):
        super().__init__()
        ...
        self.prompt = prompt
        if self.prompt:
            self.promptlayer_0 = PromptBlock(prompt_dim=512,prompt_len=5,prompt_size = 8,lin_dim = 512,
                    num_heads=16,drop_path=0.,
                    win_size=win_size,mlp_ratio=self.mlp_ratio,qkv_bias=qkv_bias,qk_scale=qk_scale,drop=drop_rate,attn_drop=attn_drop_rate,
                    norm_layer=norm_layer,token_projection=token_projection,token_mlp=token_mlp,modulator=modulator,cross_modulator=cross_modulator)
            
            self.promptlayer_1 = PromptBlock(prompt_dim=512,prompt_len=5,prompt_size = 16,lin_dim = 512,
                    num_heads=8,drop_path=0.,
                    win_size=win_size,mlp_ratio=self.mlp_ratio,qkv_bias=qkv_bias,qk_scale=qk_scale,drop=drop_rate,attn_drop=attn_drop_rate,
                    norm_layer=norm_layer,token_projection=token_projection,token_mlp=token_mlp,modulator=modulator,cross_modulator=cross_modulator)
            
            self.promptlayer_2 = PromptBlock(prompt_dim=256,prompt_len=5,prompt_size = 32,lin_dim = 256,
                    num_heads=4,drop_path=0.,
                    win_size=win_size,mlp_ratio=self.mlp_ratio,qkv_bias=qkv_bias,qk_scale=qk_scale,drop=drop_rate,attn_drop=attn_drop_rate,
                    norm_layer=norm_layer,token_projection=token_projection,token_mlp=token_mlp,modulator=modulator,cross_modulator=cross_modulator)
            
            self.promptlayer_3 = PromptBlock(prompt_dim=128,prompt_len=5,prompt_size = 64,lin_dim = 128,
                    num_heads=2,drop_path=0.,
                    win_size=win_size,mlp_ratio=self.mlp_ratio,qkv_bias=qkv_bias,qk_scale=qk_scale,drop=drop_rate,attn_drop=attn_drop_rate,
                    norm_layer=norm_layer,token_projection=token_projection,token_mlp=token_mlp,modulator=modulator,cross_modulator=cross_modulator)
        
        self.global_predictor = nn.Sequential(nn.Conv2d(embed_dim, 8, 1, 1, 0, bias=True),
                                        nn.LeakyReLU(negative_slope=0.1, inplace=True),
    ...
    def forward(self, x):
        ...
        y = self.input_proj(x)
        y = self.pos_drop(y)
        condition_global = self.global_predictor(y)
        #Encoder
        conv0 = self.encoderlayer_0(y,condition_global)
        pool0 = self.dowsample_0(conv0)
        condition_global = self.dowsample_0(condition_global)
        conv1 = self.encoderlayer_1(pool0,condition_global)
        pool1 = self.dowsample_1(conv1)
        condition_global = self.dowsample_1(condition_global)
        conv2 = self.encoderlayer_2(pool1, condition_global)
        pool2 = self.dowsample_2(conv2)
        condition_global = self.dowsample_2(condition_global)
        conv3 = self.encoderlayer_3(pool2, condition_global)
        ...
        if self.prompt:
            conv4 = self.promptlayer_0(conv4)

        #Decoder
        up0 = self.upsample_0(conv4)
        condition_global = self.upsample_0(condition_global)
        deconv0 = torch.cat([up0,conv3],-1)
        deconv0 = self.decoderlayer_0(deconv0, condition_global)

        if self.prompt:
            deconv0 = self.promptlayer_1(deconv0)
        
        up1 = self.upsample_1(deconv0)
        condition_global = self.upsample_1(condition_global)
        deconv1 = torch.cat([up1,conv2],-1)
        deconv1 = self.decoderlayer_1(deconv1, condition_global)

        if self.prompt:
            deconv1 = self.promptlayer_2(deconv1)

        up2 = self.upsample_2(deconv1)
        condition_global = self.upsample_2(condition_global)
        deconv2 = torch.cat([up2,conv1],-1)
        deconv2 = self.decoderlayer_2(deconv2, condition_global)

        if self.prompt:
            deconv2 = self.promptlayer_3(deconv2)

        up3 = self.upsample_3(deconv2)
        condition_global = self.upsample_3(condition_global)
        deconv3 = torch.cat([up3,conv0],-1)
        deconv3 = self.decoderlayer_3(deconv3, condition_global)
...
if __name__ == "__main__":
    # H, W = 512, 512
    # arch = "PromptUformer_B"

    # if arch == 'CAPromptUformer_T':
    #     model_restoration = CAPromptUformerIR(embed_dim=16,win_size=8,token_projection='linear',token_mlp='leff',modulator=True)
    # elif arch == 'CAPromptUformer_S':
    #     model_restoration = CAPromptUformerIR(embed_dim=32,win_size=8,token_projection='linear',token_mlp='leff',modulator=True)
    # elif arch == 'CAPromptUformer_S_noshift':
    #     model_restoration = CAPromptUformerIR(embed_dim=32,win_size=8,token_projection='linear',token_mlp='leff',modulator=True,
    #         shift_flag=False)
    # elif arch == 'CAPromptUformer_B_fastleff':
    #     model_restoration = CAPromptUformerIR(embed_dim=32,win_size=8,token_projection='linear',token_mlp='fastleff',
    #         depths=[1, 2, 8, 8, 2, 8, 8, 2, 1],modulator=True)  
    # elif arch == 'CAPromptUformer_B':
    #     model_restoration = CAPromptUformerIR(embed_dim=32,win_size=8,token_projection='linear',token_mlp='leff',
    #         depths=[1, 2, 8, 8, 2, 8, 8, 2, 1],modulator=True)  
    ...
    # for name, module in model_restoration.named_modules():
    #     num_params = sum(p.numel() for p in module.parameters())
    #     if num_params > 0:  # This filters out modules without parameters
    ...
    win_size = 8
    ratio = 0.5
    x = torch.randn(1, 128 * 128, 32)
    global_condition = torch.randn(1, 2, 128, 128)
    mixer = CAMixer(dim = dim,window_size=win_size,ratio=ratio,training = True)
    y, decision = mixer(x, global_condition)
    print(y.shape)
