...
class PredictorLG(nn.Module):
    ...
    def forward(self, input_x, mask=None, ratio=0.5, training = True):
...
class CAMixer(nn.Module):
    ...
    def forward(self,x,condition_global=None, mask=None, training = True):
        N,C,H,W = x.shape
...
class CATransformerBlock(nn.Module):
    ...
    def forward(self, x, global_condition=None, training = True):
        if training:
...
class CAPromptXRestormerEffv2(nn.Module):
    ...
    def forward(self, inp_img, training = True):
        if self.scale > 1:
