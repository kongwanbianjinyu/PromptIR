...
class PromptUformerIR(nn.Module):
    def __init__(self, img_size=256, in_chans=3, dd_in=3,
                 ...
                 cross_modulator=False, prompt = True, **kwargs):
        super().__init__()
    ...
    def forward(self, x, mask=None):
        ...
        if self.prompt:
            conv4 = self.promptlayer_0(conv4,mask=mask)
        ...
        if self.prompt:
            deconv0 = self.promptlayer_1(deconv0,mask=mask)
        ...
        if self.prompt:
            deconv1 = self.promptlayer_2(deconv1,mask=mask)
        ...
        if self.prompt:
            deconv2 = self.promptlayer_3(deconv2,mask=mask)
...
if __name__ == "__main__":
    ...
    if arch == 'PromptUformer_T':
        model_restoration = PromptUformerIR(embed_dim=16,win_size=8,token_projection='linear',token_mlp='leff',modulator=True)
    elif arch == 'PromptUformer_S':
        model_restoration = PromptUformerIR(embed_dim=32,win_size=8,token_projection='linear',token_mlp='leff',modulator=True)
    elif arch == 'PromptUformer_S_noshift':
        model_restoration = PromptUformerIR(embed_dim=32,win_size=8,token_projection='linear',token_mlp='leff',modulator=True,
            shift_flag=False)
    elif arch == 'PromptUformer_B_fastleff':
        model_restoration = PromptUformerIR(embed_dim=32,win_size=8,token_projection='linear',token_mlp='fastleff',
            depths=[1, 2, 8, 8, 2, 8, 8, 2, 1],modulator=True)  
    elif arch == 'PromptUformer_B':
        model_restoration = PromptUformerIR(embed_dim=32,win_size=8,token_projection='linear',token_mlp='leff',
            depths=[1, 2, 8, 8, 2, 8, 8, 2, 1],modulator=True)
